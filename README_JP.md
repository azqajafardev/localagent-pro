...
> **`SEARXNG_PORT` と `SEARXNG_BASE_URL` に関する注意**: `SEARXNG_PORT` はDockerがバインドするホストポートのみを変更します（例：`8001`）。searxngコンテナは内部的には常にポート `8080` でリッスンします。したがって、Docker内で完全に実行する場合、`SEARXNG_BASE_URL` は常に `http://searxng:8080`（内部Dockerネットワークアドレス）のままにしてください。ホストでCLIモードを実行する場合のみ、`SEARXNG_BASE_URL` を変更してください。
...
> **ポート競合に関する注意**: ホスト上でポート `8080` が既に使用されている場合は、`.env` の `SEARXNG_PORT` を変更してください（例：`8001`）。`SEARXNG_BASE_URL` は `http://searxng:8080` のままにしてください — 内部Dockerポートは変更されません。
